...
if [ "$colour_support" = yes ]; then
	...
	if [[ -n "$custom_colour_profile" ]]; then
		...
		if [[ -d "$profile_dir" ]] && [[ -e "${colours_dir}/set_colour_variables.py" ]]; then
			eval "$("${colours_dir}/set_colour_variables.py" "${profile_dir}/ls_classes.yaml" "${profile_dir}/colours.yaml")"
		fi
